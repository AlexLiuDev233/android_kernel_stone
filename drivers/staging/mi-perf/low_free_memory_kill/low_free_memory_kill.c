...
extern bool ksu_boot_completed __read_mostly;
static bool lmk_inited __read_mostly = false;
...
static const struct file_operations event_proc_ops = {
	.open = lmk_event_open,
	.poll = lmk_event_poll,
	.read = seq_read,
	.llseek = seq_lseek,
...
static int lmk_wait_boot_thread(void *data)
{
	...
	if (!pg_memory->kobj) {
		pr_err("lfm: sysfs create failed\n");
		return -ENOMEM;
	}

	pg_memory->enable = FUNC_ON;
	lmk_inited = true;
	pr_info("lfm: initialized after boot completed\n");

	return 0;
}
static struct task_struct *lmk_boot_task;
...
static int __init low_free_memory_kill_init(void)
{
	...
	lmk_boot_task = kthread_run(lmk_wait_boot_thread,NULL,"lfm_wait_boot");

	if (IS_ERR(lmk_boot_task)) {
		pr_err("lfm: failed to create wait thread\n");
		return RET_FAIL;
	}

	pr_info("lfm: wait for boot completed\n");
