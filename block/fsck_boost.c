 ...
 * Description: prefetch the meta data of userdata partition to speed up the
...
struct gendisk *is_fsck_boost_done(const struct block_device *bdev)
{
    ...
    if (strcmp(disk->disk_name, bdev->bd_disk->disk_name) || partno != bdev->bd_partno) {
...
static int read_pagecaches(void *info)
{
    ...
    if (get_fbi(&disk, &partno))
        return -1;

    bdev = blkdev_get_by_dev(MKDEV(disk->major, disk->first_minor + partno), FMODE_READ, NULL);
    if (IS_ERR(bdev)) {
        pr_err("%s blkdev_get_by_dev fail! err=%ld\n", __func__, PTR_ERR(bdev));
        return PTR_ERR(bdev);
    }
    ...
    for (i = 0; i < g_fbi.seg_num; i++) {
        ...
        page_cache_sync_readahead(mapping, NULL, NULL,
                                  PREFETCH_SEGMENT_SIZE * i, PREFETCH_SEGMENT_SIZE);
    }
...
#if defined(CONFIG_HISI_EAS_SCHED) || defined(CONFIG_ARCH_HW)
    struct cpumask slow_cpus;
#ifdef CONFIG_HISI_EAS_SCHED
    get_slow_cpus(&slow_cpus);
#elif defined(CONFIG_ARCH_HW)
                      ...
                      int partno, sector_t len)
{
    ...
    if (!info || strcmp(info->volname, "userdata"))
        return;
#if defined(CONFIG_MAS_CMDLINE_PARSE) || defined(CONFIG_HISI_CMDLINE_PARSE)
